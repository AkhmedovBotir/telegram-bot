...
const userHandler = {
  ...
  handleStart: async (ctx) => {
    try {
      ...
      await ctx.reply(
        "Xatolik yuz berdi. Iltimos, /start komandasini yuborib qayta urinib ko'ring.",
      );
    }
  },
  ...
  handlePhoto: async (ctx) => {
    try {
      ...
      if (user.state === 'expired' || user.state === 'waiting_payment') {
        ...
        // Save image info to user with full path
        user.paymentImage = `/uploads/${fileName}`;
        user.paymentDate = date;
        user.state = "waiting_approval";
        await user.save();

        await ctx.reply(
          "To'lov cheki qabul qilindi. Administrator tekshirishi kutilmoqda.",
          Markup.removeKeyboard()
        );

        // Notify admin about new payment with photo
        if (config.ADMIN_USER_ID) {
          await ctx.telegram.sendPhoto(config.ADMIN_USER_ID, photo.file_id, {
            caption: `Yangi to'lov cheki:\nFoydalanuvchi: ${user.fullName}\nTelefon: ${user.phoneNumber}\nID: ${user.telegramId}\nSana: ${date.toLocaleString()}`
          });
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (user.state === "waiting_name") {
        ...
        if (user.accessGranted && user.state === "active") {
          ...
          await ctx.reply(
            "Iltimos, guruhga kirish uchun administrator tasdiqlashini kuting.",
          );
        }
      }
    } catch (error) {
      ...
      await ctx.reply(
        "Xatolik yuz berdi. Iltimos, qayta urinib ko'ring yoki /start orqali qayta boshlang.",
      );
    }
  },
  ...
  handleContact: async (ctx) => {
    try {
      ...
      if (contact.user_id !== telegramId) {
        return ctx.reply(
          "Iltimos, boshqa foydalanuvchining emas, o'zingizning kontakt ma'lumotingizni yuboring.",
        );
      }
      ...
      await ctx.reply(
        "Kontakt ma'lumotlaringizni qayta ishlashda xatolik yuz berdi. Iltimos, qayta urinib ko'ring.",
      );
    }
  },
};
...
async function processName(ctx, user) {
  ...
  if (fullName.length < 3) {
    return ctx.reply(
      "Iltimos, kamida 3 ta belgidan iborat to'g'ri to'liq ismingizni kiriting.",
    );
  }
...
async function processPhoneText(ctx, user) {
  ...
  if (phoneNumber.length < 7) {
    return ctx.reply(
      "Iltimos, to'g'ri telefon raqamni kiriting yoki \"Telefon raqamni yuborish\" tugmasidan foydalaning.",
    );
  }
...
async function processRegistrationComplete(ctx, user) {
  // Set the access expiry date (10 seconds from now)
  ...
  const completeSetting = await Setting.findOne({
    key: "text_registration_complete",
  });
  const completeText = completeSetting
    ? completeSetting.value
    : `Ro'yxatdan o'tganingiz uchun rahmat! Sizga ${config.FREE_ACCESS_DAYS} kunlik bepul guruh a'zoligi berildi.`;

  await ctx.reply(completeText, Markup.removeKeyboard());
...
async function sendGroupInvite(ctx, user) {
  try {
    ...
    if (user.state !== "active" || !user.accessGranted) {
      return ctx.reply(
        "Guruhga qo'shilish uchun admin tasdiqini kutishingiz kerak.",
      );
    }
    ...
    if (isMember) {
      // If user is already in group, remove their invite link and show status
      if (user.inviteLink) {
        user.inviteLink = null;
        await user.save();
      }
      const daysLeft = Math.ceil(
        (new Date(user.accessExpiryDate) - new Date()) / (1000 * 60 * 60 * 24),
      );
      await ctx.reply(
    ...
    const inviteText = inviteSetting
      ? inviteSetting.value
    ...
    if (config.GENERATE_NEW_INVITE_LINKS && config.GROUP_ID) {
      try {
        ...
        if (newLink) {
          ...
          console.warn(
            `Failed to generate new invite link for user ${user.telegramId}, using default link`,
          );
        }
      } catch (linkError) {
    ...
    await ctx.reply(
      inviteText,
      Markup.inlineKeyboard([
        [Markup.button.url('Guruhga qo\'shilish', inviteLink)],
        [Markup.button.callback('Muddat holatini tekshirish', 'check_expiry')]
      ])
    );
  } catch (error) {
    ...
    await ctx.reply(
      "Guruh havola yaratishda xatolik yuz berdi. Iltimos, keyinroq qayta urinib ko'ring.",
    );
  }
}
...
async function handleExpiredAccess(ctx, user) {
  try {
    const expiredSetting = await Setting.findOne({
      key: "text_access_expired",
    });
    const expiredText = expiredSetting
      ? expiredSetting.value
...
async function requestAccess(ctx) {
  try {
    ...
    const requestSetting = await Setting.findOne({
      key: "text_request_access",
    });
    const requestText = requestSetting
      ? requestSetting.value
    ...
    await ctx.reply(
      "So'rovni yuborishda xatolik yuz berdi. Iltimos, keyinroq qayta urinib ko'ring.",
    );
  }
}
...
async function checkExpiryStatus(ctx) {
  try {
    ...
    await ctx.reply(
      ...
      Markup.inlineKeyboard([Markup.button.url("Guruhga o'tish", inviteLink)]),
    );
  } catch (error) {
    console.error("Error checking expiry status:", error);
    await ctx.reply(
      "Holatni tekshirishda xatolik yuz berdi. Iltimos, keyinroq qayta urinib ko'ring.",
    );
  }
}
